...
-- 1) 名单表（人员分组，如「1班」「2班」「兴趣小组」）
create table if not exists lists (
  id         uuid primary key default gen_random_uuid(),
  name       text not null,                 -- 名单名称
  ord        int  not null default 0,       -- 顺序
...
create table if not exists people (
  ...
  list_id    uuid,                          -- 所属名单 id
  ord        int  not null default 0,       -- 显示顺序（按录入顺序）
  created_at timestamptz not null default now()
);

-- 3) 签到活动表（每次「日期 + 名称/时段」对应一条）
...
-- 5) 授权：让匿名(anon)角色可读写这些表
--    （本工具把 anon key 直接内嵌在前端，适合班级/内部使用；
--      对公网恶意者而言 key 可见，请勿存放敏感数据）
grant usage on schema public to anon, authenticated;
grant select, insert, update, delete on lists, people, sessions, records to anon, authenticated;
